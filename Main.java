		...
		do {
			try {
				...
				if (choice == 1) {
					...
					myBox.add(p); //could throw exception here and skip the rest of the code. 
					System.out.println("\n" + name + " added!");
				} else if (choice == 2) {
					System.out.println(myBox);
				} else if (choice == 3) {
			...
			} catch(IllegalArgumentException paee) {
				System.out.println("\nInvalid name in type #1 and/or type #2 entered, Please try again.");
				System.out.println("\n(Arrays,toString(Pokemon.TYPES))");
